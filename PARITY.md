...
| Standalone ambilight | **Done.** The glow is computed as before for the room and now shown without it: as a quad layer behind a flat screen (Windows' glow layer) and in the curve pass round a curved one; the desktop app's Ambilight checkbox and strength (`VRXL 6`) apply live and, as on Windows, also control the room's glow. The curve and room passes with the glow match their CPU references. Linux defaults it on. | None. |
| World colour | **Done.** As on Windows: the room's shading with the room on, the curve pass's background round a curved screen, and a one-colour projection layer behind a flat screen with Room 0. The desktop app's presets and #RRGGBB box (`VRXL 7`) apply live. The curve pass matches its CPU reference with a world colour. | None. |
| Screen follows the head | **Done.** As in xrapp5: the screen is re-placed in front of the headset every frame, and the room rests (it needs a fixed screen); the curve, glow and world colour still apply. The desktop app's checkbox (`VRXL 8`) applies live and rests the room controls ("Needs the fixed screen"), as in the WPF app. 90 fps either way in the synthetic test. | None. |
| Steady depth | **Done, with a different motion source.** xrapp5's steadying maths (`depth_fusion.h`) runs on the depth worker between normalising and dilating each map, as on Windows. Windows' motion vectors come from D3D12 video motion estimation, which Vulkan lacks; Linux finds the same 8×8 quarter-pel vectors with a CPU coarse-to-fine block search (`block_motion.h`, about 6 ms; 99–100 % exact on known moves) on the frame at the depth grid's size. `depth_fusion.h` gained an opt-in std::thread row loop (FUSION_STD_THREADS) as libstdc++'s parallel algorithms need TBB. On by default, as on Windows. | A GPU block search if the CPU cost ever limits the depth rate. |
