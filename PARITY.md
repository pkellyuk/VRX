...
| Standalone ambilight | **Done.** The glow is computed as before for the room and now shown without it: as a quad layer behind a flat screen (Windows' glow layer) and in the curve pass round a curved one; the desktop app's Ambilight checkbox and strength (`VRXL 6`) apply live and, as on Windows, also control the room's glow. The curve and room passes with the glow match their CPU references. Linux defaults it on. | The world colour behind the screen is not ported. |
