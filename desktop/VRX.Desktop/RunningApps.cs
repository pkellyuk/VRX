...
public static class RunningApps
{
    ...
    public static List<RunningApp> List(bool showAll)
    {
        ...
        EnumWindows((hwnd, _) =>
        {
            ...
            if (cls.ToString() is "ConsoleWindowClass" or "CASCADIA_HOSTING_WINDOW_CLASS") return true;
        ...
        foreach (var process in Process.GetProcesses())
        {
            using (process)
            {
                try
                {
                    ...
                    if (name.ToLowerInvariant() is "xrapp5.exe" or "xrplayer.exe" or "windowsterminal.exe" or "conhost.exe" or "openconsole.exe" or "cmd.exe" or "powershell.exe" or "pwsh.exe") gameWindows = [];
