...
// "VRXL 6" line, culture-independent. `recenter` counts Recenter requests in
...
// the ambilight (0 or 1) and its strength (0..100).
public static class LiveSettings
{
    public static string Snapshot(LinuxProfile profile, uint recenter = 0)
    {
        ...
        return string.Create(CultureInfo.InvariantCulture,
            $"VRXL 6 {p.Width:F3} {p.Distance:F3} {p.Height:F3} {p.Horizontal:F3} {p.Strength:F3} " +
            $"{p.Room} {p.Glass} {p.Reflect} {p.Light} {p.LightRgb} {recenter} {p.Timing} {p.Curve} " +
            $"{(p.Ambilight ? 1 : 0)} {p.AmbilightStrength}\n");
    }
