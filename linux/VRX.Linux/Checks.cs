...
public static class Checks
{
    public static int Run()
    {
        ...
        Expect(LiveSettings.Snapshot(LinuxProfile.Default) ==
               "VRXL 8 2.000 2.000 0.000 0.000 1.000 30 60 25 30 16757867 0 1 0 1 85 0 0\n", "default VRXL 8 snapshot (delayed, flat, glow, black, fixed)");
        Expect(LiveSettings.Snapshot(LinuxProfile.Default, 7).EndsWith(" 16757867 7 1 0 1 85 0 0\n"), "recenter counter in the snapshot");
        Expect(LiveSettings.Snapshot(LinuxProfile.Default with { Timing = LinuxProfile.TimingMatched }).EndsWith(" 0 2 0 1 85 0 0\n"),
               "frame timing in the snapshot");
        Expect(LiveSettings.Snapshot(LinuxProfile.Default with { Curve = 65 }).EndsWith(" 1 65 1 85 0 0\n"), "screen curve in the snapshot");
        Expect(LiveSettings.Snapshot(LinuxProfile.Default with { Ambilight = false, AmbilightStrength = 40 }).EndsWith(" 0 0 40 0 0\n"),
               "ambilight in the snapshot");
        Expect(LiveSettings.Snapshot(LinuxProfile.Default with { WorldColor = "#33415c" }).EndsWith(" 1 85 3359068 0\n"),
               "world colour in the snapshot");
        Expect(LiveSettings.Snapshot(LinuxProfile.Default with { Follow = true }).EndsWith(" 0 1\n"), "follow in the snapshot");
        ...
        try
        {
            ...
            Expect(LiveSettings.Snapshot(LinuxProfile.Default with { Width = 2.5 }).StartsWith("VRXL 8 2.500 "),
        ...
        try
        {
            ...
            var saved = new SortedDictionary<string, LinuxProfile>(StringComparer.Ordinal)
            {
                ...
                ["Game"] = LinuxProfile.Default with { Cuda = false, Width = 3.25, Room = 0, LightColor = "#102030",
                                                       ...
                                                       Ambilight = false, AmbilightStrength = 30, WorldColor = "#0B1530", Follow = true },
            };
            ...
            Expect(old.Width == 3.0 && old.Room == LinuxProfile.Default.Room && old.Timing == LinuxProfile.TimingDelayed && old.Curve == 0 && old.Ambilight && old.WorldColor == "#000000" && !old.Follow,
