...
if (!$OutputRoot) { $OutputRoot = Join-Path $PSScriptRoot ('out\v1.2.0-' + (Get-Date -Format 'yyyyMMdd-HHmmss')) }
$OutputRoot = [IO.Path]::GetFullPath($OutputRoot)
$payload = Join-Path $OutputRoot 'VRX-1.2.0-win-x64'
...
try {
    ...
    @("VRX 1.2.0", "Source commit: $commit", "Working tree dirty: $([bool]$dirty)", "Built UTC: $([DateTime]::UtcNow.ToString('O'))") | Set-Content "$payload\BUILD.txt"
    ...
    Compress-Archive -LiteralPath $payload -DestinationPath "$OutputRoot\VRX-1.2.0-win-x64.zip" -CompressionLevel Optimal
