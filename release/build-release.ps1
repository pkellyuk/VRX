...
if (!$OutputRoot) { $OutputRoot = Join-Path $PSScriptRoot ('out\v1.5.0-' + (Get-Date -Format 'yyyyMMdd-HHmmss')) }
$OutputRoot = [IO.Path]::GetFullPath($OutputRoot)
$payload = Join-Path $OutputRoot 'VRX-1.5.0-win-x64'
...
try {
    ...
    Compress-Archive -LiteralPath $payload -DestinationPath "$OutputRoot\VRX-1.5.0-win-x64.zip" -CompressionLevel Optimal
