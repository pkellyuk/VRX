...
`VRXL 2` fields, `VRXL 4` the game frame timing (latest, delayed, matched) and `VRXL 5` the screen curve (0–100) and `VRXL 6` the ambilight (on/off and strength) and `VRXL 7` the world colour. `VRXL 2` appends Room, Glass, Reflections,
ceiling light and RGB light colour. The desktop app writes version 7 and can
